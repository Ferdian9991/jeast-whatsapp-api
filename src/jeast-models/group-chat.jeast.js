...
class GroupChat extends Chat {
  ...
  async addParticipants(participantIds) {
    return await this.client.clientPage.evaluate(
      (chatId, participantIds) => {
        ...
        return window.Store.GroupParticipants.sendAddParticipants(
          chatWid,
          participantWids
        );
      },
  ...
  async removeParticipants(participantIds) {
    return await this.client.clientPage.evaluate(
      (chatId, participantIds) => {
        ...
        return window.Store.GroupParticipants.sendRemoveParticipants(
          chatWid,
          participantWids
        );
      },
  ...
  async promoteParticipants(participantIds) {
    return await this.client.clientPage.evaluate(
      (chatId, participantIds) => {
        ...
        return window.Store.GroupParticipants.sendPromoteParticipants(
          chatWid,
          participantWids
        );
      },
  ...
  async demoteParticipants(participantIds) {
    return await this.client.clientPage.evaluate(
      (chatId, participantIds) => {
        ...
        return window.Store.GroupParticipants.sendDemoteParticipants(
          chatWid,
          participantWids
        );
      },
  ...
  async setSubject(subject) {
    const success = await this.client.clientPage.evaluate(
      async (chatId, subject) => {
        const chatWid = window.Store.WidFactory.createWid(chatId);
        try {
          return await window.Store.GroupUtils.sendSetGroupSubject(
            chatWid,
            subject
          );
        } catch (err) {
  ...
  async setDescription(description) {
    const success = await this.client.clientPage.evaluate(
      async (chatId, description) => {
        ...
        try {
          return await window.Store.GroupUtils.sendSetGroupDescription(
            chatWid,
            description,
  ...
  async setMessagesAdminsOnly(adminsOnly = true) {
    const success = await this.client.clientPage.evaluate(
      async (chatId, adminsOnly) => {
        const chatWid = window.Store.WidFactory.createWid(chatId);
        try {
          return await window.Store.GroupUtils.sendSetGroupProperty(
            chatWid,
            "announcement",
            adminsOnly ? 1 : 0
          );
   ...
   */
  async getInviteCode() {
    const code = await this.client.clientPage.evaluate(async (chatId) => {
      const chatWid = window.Store.WidFactory.createWid(chatId);
   ...
   */
  async revokeInvite() {
    const code = await this.client.clientPage.evaluate((chatId) => {
      const chatWid = window.Store.WidFactory.createWid(chatId);
   ...
   */
  async leave() {
    await this.client.clientPage.evaluate((chatId) => {
      const chatWid = window.Store.WidFactory.createWid(chatId);
