...
exports.LoadModule = () => {
  ...
  window.JWeb.sendSeen = async (chatId) => {
  ...
  window.JWeb.sendMessage = async (chat, content, options = {}) => {
    let attOptions = {};
    if (options.attachment) {
      attOptions = options.sendMediaAsSticker
        ? await window.JWeb.processStickerData(options.attachment)
        : await window.JWeb.processMediaData(options.attachment, {
  ...
  window.JWeb.toStickerData = async (mediaInfo) => {
    if (mediaInfo.mimetype == "image/webp") return mediaInfo;

    const file = window.JWeb.mediaInfoToFile(mediaInfo);
    const webpSticker = await window.Store.StickerTools.toWebpSticker(file);
    const webpBuffer = await webpSticker.arrayBuffer();
    const data = window.JWeb.arrayBufferToBase64(webpBuffer);
  ...
  window.JWeb.processStickerData = async (mediaInfo) => {
  ...
  window.JWeb.processMediaData = async (
    mediaInfo,
  ...
  window.JWeb.getMessageModel = (message) => {
  ...
  window.JWeb.getChatModel = async (chat) => {
  ...
  window.JWeb.getChat = async (chatId) => {
    const chatWid = window.Store.WidFactory.createWid(chatId);
    const chat = await window.Store.Chat.find(chatWid);
    return await window.JWeb.getChatModel(chat);
  };

  window.JWeb.getChats = async () => {
    const chats = window.Store.Chat.models;

    const chatPromises = chats.map((chat) => window.JWeb.getChatModel(chat));
    return await Promise.all(chatPromises);
  };

  window.JWeb.getContactModel = (contact) => {
  ...
  window.JWeb.getContact = async (contactId) => {
    const wid = window.Store.WidFactory.createWid(contactId);
    const contact = await window.Store.Contact.find(wid);
    return window.JWeb.getContactModel(contact);
  };

  window.JWeb.getContacts = () => {
    const contacts = window.Store.Contact.models;
    return contacts.map((contact) => window.JWeb.getContactModel(contact));
  };

  window.JWeb.mediaInfoToFile = ({ data, mimetype, filename }) => {
  ...
  window.JWeb.arrayBufferToBase64 = (arrayBuffer) => {
    let binary = "";
  ...
  window.JWeb.getFileHash = async (data) => {
  ...
  window.JWeb.sendClearChat = async (chatId) => {
    let chat = window.Store.Chat.get(chatId);
    if (chat !== undefined) {
  ...
  window.JWeb.sendDeleteChat = async (chatId) => {
    let chat = window.Store.Chat.get(chatId);
    if (chat !== undefined) {
  ...
  window.JWeb.sendChatstate = async (state, chatId) => {
    if (window.Store.Features.features.MD_BACKEND) {
  ...
  window.JWeb.getLabelModel = (label) => {
  ...
  window.JWeb.getLabels = () => {
    const labels = window.Store.Label.models;
    return labels.map((label) => window.JWeb.getLabelModel(label));
  };

  window.JWeb.getLabel = (labelId) => {
    const label = window.Store.Label.get(labelId);
    return window.JWeb.getLabelModel(label);
  };

  window.JWeb.getChatLabels = async (chatId) => {
    const chat = await window.JWeb.getChat(chatId);
    return (chat.labels || []).map((id) => window.JWeb.getLabel(id));
  };

  window.JWeb.getOrderDetail = async (orderId, token, chatId) => {
    const chatWid = window.Store.WidFactory.createWid(chatId);
    return window.Store.QueryOrder.queryOrder(chatWid, orderId, 80, 80, token);
  };

  window.JWeb.getProductMetadata = async (productId) => {
